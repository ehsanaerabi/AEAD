...
int main()
{
   ...
   // unsigned long long s[500];
    ...
    unsigned char ciphertext[msglen+adlen+taglen];
    unsigned char key[16];
    unsigned char iv[16];
    unsigned char mac[taglen];
    unsigned long long   clen;    // msg, adlen, clen in bytes.
  //  unsigned char maclen = taglen;
   ...
   // adlen = 1024;
    ...
    for (i = 0; i < msglen+adlen+taglen; i++) ciphertext[i] = 0;
    ...
    t = crypto_aead_decrypt(plaintext,&decryptedMsgLen,0,ciphertext,clen,ad,adlen,iv,key);
    ...
    for( i = 0; i < msglen; i++) printf(" %2x", plaintext[i]);
        	   printf("\n\nThe AD-text is: \n");
    for( i = 0; i < adlen; i++) printf(" %2x", ad[i]);
